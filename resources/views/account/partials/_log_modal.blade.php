@if(isset($accounts) && $accounts != null)
    @foreach($accounts as $account)
        <div class="modal" id="activity_account_{{ $account->id }}" tabindex="-1">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="accordion" id="template_accordion">
                            @if(isset($account->activities) && !$account->activities->isEmpty())
                                @foreach($account->activities()->orderBy('created_at', 'desc')->get() as $activity)
                                    @if($activity->subject_id == $account->id)
                                        <div class="card">
                                    ...
                                    <span class="float-left">
                                        <b>{{ $activity->causer->name }}</b> {{ $activity->description }} variable for template <b>#{{ $activity->subject->type }}</b>
                                    </span>
