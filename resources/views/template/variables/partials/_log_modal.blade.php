        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="accordion" id="template_accordion">
                    @if(isset($variable->activities) && !$variable->activities->isEmpty())
                        @foreach($variable->activities()->orderBy('created_at', 'desc')->get() as $activity)
                            @if($activity->subject_id == $variable->id)
                    <div class="card">
                            ...
                            <h2 class="mb-0">
                                <button class="btn btn-link btn-block text-left" type="button" data-toggle="collapse" data-target="#activity_{{ $activity->id }}" aria-expanded="true" aria-controls="collapseOne">
                                    <span class="float-left">
                                        <b>{{ $activity->causer->name }}</b> {{ $activity->description }} variable for template <b>#{{ $activity->subject->template->id }}</b>
                                    </span>
