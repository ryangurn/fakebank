@if(isset($template->routes) && !$template->routes->isEmpty())
    @foreach($template->routes as $route)
        <div class="modal" id="activity_route_{{ $route->id }}" tabindex="-1">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="accordion" id="template_accordion">
                            @if(isset($route->activities) && !$route->activities->isEmpty())
                                @foreach($route->activities()->orderBy('created_at', 'desc')->get() as $activity)
                                    @if($activity->subject_id == $route->id)
                                        <div class="card">
                                                ...
                                                <h2 class="mb-0">
                                                    <button class="btn btn-link btn-block text-left" type="button" data-toggle="collapse" data-target="#activity_{{ $activity->id }}" aria-expanded="true" aria-controls="collapseOne">
                                                        <span class="float-left">
                                                            <b>{{ $activity->causer->name }}</b> {{ $activity->description }} route <b>{{ $activity->subject->route }}</b>
                                                        </span>
